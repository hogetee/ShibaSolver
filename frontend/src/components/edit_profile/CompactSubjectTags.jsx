...
export default function CompactSubjectTags({ subjects, onRemove }) {
    ...
    <div className="flex flex-wrap gap-1">
      {subjects.map((subject, index) => {
        ...
        return (
          <div
            key={index}
            className={`${subjectColor.bg} ${subjectColor.text} px-2 py-1 rounded-full text-xs flex items-center gap-1`}
          >
            <span>{subjectName}</span>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation(); // Prevent dropdown from opening when clicking remove
                onRemove(subject);
              }}
              className={`${subjectColor.button} text-xs`}
            >
              ×
            </button>
          </div>
